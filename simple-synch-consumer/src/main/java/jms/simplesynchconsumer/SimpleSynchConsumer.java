package jms.simplesynchconsumer;
...
//  private JMSProducer messageProducer = null;
    ...
    public void connect() {
    	...
    	try {
			...
			messageConsumer=s.createConsumer(destination);
...
//    }
    public void disconnect() {
        if (s != null && c!= null) {
        	...
        	logger.info(this.toString() + ": Disconnected (" + messagesReceived + " message(s) sent)");
        }
    }
    ...
    public void start() {
    	try {
			...
			logger.info("Error starting message reception :"+ e.toString());
			e.printStackTrace();
		}
...
//        context.start();
    ...
    public void stop() {
        ...
        try {
			...
			logger.info("Error stoping message reception" + e.toString());
		}
